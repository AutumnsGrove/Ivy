...
export const GET: RequestHandler = async ({ params, locals, platform }) => {
  ...
  try {
    ...
    // Mark as read
    if (row.is_read === 0) {
      await env.DB.prepare("UPDATE ivy_emails SET is_read = 1 WHERE id = ?")
        .bind(params.id)
        .run();
    }
...
export const PUT: RequestHandler = async ({
  ...
  try {
    ...
    const result = await env.DB.prepare(
      `UPDATE ivy_emails SET ${updates.join(", ")} WHERE id = ?`,
    )
      .bind(...bindings)
      .run();
...
export const DELETE: RequestHandler = async ({ params, locals, platform }) => {
  ...
  try {
    ...
    const email = await env.DB.prepare(
      "SELECT r2_content_key FROM ivy_emails WHERE id = ?",
    )
      .bind(params.id)
    ...
    // Delete from D1
    await env.DB.prepare("DELETE FROM ivy_emails WHERE id = ?")
      .bind(params.id)
      .run();
