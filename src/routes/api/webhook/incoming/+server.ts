...
export const POST: RequestHandler = async ({ request, platform }) => {
	...
	try {
       ...
       VALUES (?, ?, ?, ?, 'pending', 0, ?)`
		...
		// 9. TRIGGER ASYNC PROCESSING (non-blocking)
		// TODO: Use Durable Object or Queue for processing
		// For now, we'll process in the background via a separate worker
		// platform?.context?.waitUntil(processBufferEntry(env, bufferId));
